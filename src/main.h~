...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.01 * COIN;	// 1% pos
...
public:
    ...
    {
        // graffiti: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // graffiti: get transaction coin age
...
public:
    ...
    // graffiti: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // graffiti: two types of block: proof-of-work or proof-of-stake
    ...
    // graffiti: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // graffiti: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // graffiti: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // graffiti: block index flags
    enum  
    {
